...
class LegalExpertState(MessagesState):
    ...
    Attributes:
        ...
        retrieved (list[dict]): Slim per-doc metadata from the most recent
            retrieval call (case_id, paragraph_id, title, court, year, score).
            Excludes page_content to keep checkpoint payloads small — the
            in-process _retrieved_docs contextvar carries the full text for
            consumers that need it. Optional — older checkpoints lack this key.
